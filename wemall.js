...
function reduceProductN ( wemallId ){
	...
	var cartMenuN = parseFloat($('#totalNum').html())-1;
	$('#totalNum').html( cartMenuN );
	$('#cartN2').html( cartMenuN );
...
function doProduct (id , name , price,num) {
	...
	if (bgcolor == '#FFFFFF') {
		...
		var html = '<li class="ccbg2" id="'+wemallId+'"><div class="orderdish"><span name="title">'+name+'</span><span class="price" id="v_0">'+price+'</span><span class="price">元</span></div><div class="orderchange"><a href=javascript:addProductN("'+wemallId+'_'+price+'_'+num+'") class="increase"><b class="ico_increase">加一份</b></a><span class="count" id="num_1_499">1</span><a href=javascript:reduceProductN("'+wemallId+'_'+price+'") class="reduce"><b class="ico_reduce">减一份</b></a></div></li>';
		$('#ullist').append(html);
	}else{
...
//		json=json_encode($json);
		...
		$.ajax({
			type:'POST',
			url:appurl+'/App/Index/fetchgooddetail2',
			data : {
//				name : $_GET['name'],
//				num:num
//				menu_id:menu_id
			...
			success : function(response , status , xhr){
				...
				var left=json2.num-num2;
				$('#totalNum').html(left);

				$.ajax({
						type:'POST',
						url:appurl+'/App/Index/saveleftnum',
						data:{
							name:name,
							num:left,
						},
